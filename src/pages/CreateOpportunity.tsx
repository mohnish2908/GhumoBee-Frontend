...
import { createOpportunity, getOpportunityById } from "../services/operations/opportunityApi"
...
const CreateOpportunity: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && editId) {
        ...
        try {
          await dispatch(updateOpportunityData({ id: editId, data: submissionData })).unwrap();
  ...
  const toggleArrayValue = (arrayName: keyof typeof formData, value: string) => {
    setFormData(prev => {
        ...
        >
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-lg">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Your volunteer opportunity has been successfully created and is now live on GhumoBee. 
              Volunteers can start applying immediately.
            </p>
    ...
    return (
      <div className="min-h-screen pt-16 flex items-center justify-center bg-white dark:bg-gray-900">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading opportunity data...</p>
    ...
    return (
      <div className="min-h-screen pt-16 flex items-center justify-center bg-white dark:bg-gray-900">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">
            Please wait while we {isEditMode ? 'update' : 'create'} your opportunity
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen pt-16">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        >
          {/* Basic Information */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold text-charcoal dark:text-white mb-6">Basic Information</h2>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  <span className={`text-sm font-medium ${
                    formData.isActive ? 'text-gray-500' : 'text-charcoal dark:text-white'
                  }`}>
                    Inactive
                  </span>
                  <button
          ...
          {/* Property Type */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold text-charcoal dark:text-white mb-4">Property Type</h2>
            <p className="text-gray-600 dark:text-gray-300 mb-4">Select the type(s) that best describe your property</p>
          ...
          {/* Skills Needed */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold text-charcoal dark:text-white mb-4">Skills Needed</h2>
            <p className="text-gray-600 dark:text-gray-300 mb-4">Select the skills that would be helpful for this opportunity</p>
            <div className="flex flex-wrap gap-2">
              {(showAllSkills ? availableSkills : availableSkills.slice(0, 7)).map((skill) => (
                <button
                  key={skill}
                  type="button"
          ...
          <div className="mb-8">
            <h2 className="text-xl font-semibold text-charcoal dark:text-white mb-6">Duration & Working Hours</h2>
          ...
          <div className="mb-8">
            <h2 className="text-xl font-semibold text-charcoal dark:text-white mb-6">Accommodation & Benefits</h2>
                ...
                {mealOptions.map((meal) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-center">
                        <IconComponent className="h-5 w-5 mx-auto mb-1" />
                        <span className="text-xs font-medium block">{meal.name}</span>
                      </div>
                    </label>
                  );
                })}
                ...
                {amenities.map((amenity) => {
                  ...
                  return (
                    <button
                      key={amenity.id}
                      type="button"
                      onClick={() => toggleArrayValue('amenities', amenity.name)}
                      className={`p-3 rounded-lg border-2 transition-all ${
                    ...
                    >
                      <IconComponent className="h-5 w-5 mx-auto mb-1" />
                      <span className="text-xs font-medium block text-center">{amenity.name}</span>
                    </button>
                  );
                })}
              </div>
                ...
                {transportOptions.map((transport) => {
                  ...
                  return (
                    <button
                      key={transport.id}
                      type="button"
                      onClick={() => toggleArrayValue('transport', transport.name)}
                      className={`p-3 rounded-lg border-2 transition-all ${
                    ...
                    >
                      <IconComponent className="h-5 w-5 mx-auto mb-1" />
                      <span className="text-xs font-medium block text-center">{transport.name}</span>
                    </button>
                  );
                })}
              </div>
          ...
          {/* Photos */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold text-charcoal dark:text-white mb-4">Photos</h2>
            <p className="text-gray-600 dark:text-gray-300 mb-4">
              Upload 1-3 photos of your location, accommodation, and activities (Required: minimum 1, maximum 3)
            </p>
            ...
            {allImages.length < 3 && (
              <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 text-center mb-4">
                ...
                <h3 className="text-lg font-medium text-charcoal dark:text-white mb-2">Upload Photos</h3>
                <p className="text-gray-600 dark:text-gray-300 mb-4">
                  {allImages.length === 0 
                    ? "Add photos of your location, accommodation, and activities" 
                    : `Add ${3 - allImages.length} more photo${3 - allImages.length > 1 ? 's' : ''}`
                  }
                </p>
